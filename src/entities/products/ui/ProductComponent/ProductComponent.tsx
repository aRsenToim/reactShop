    ...
    return <div className={s.Product}>
        ...
        <div className={s.Product__info}>
            ...
            <div className={s.Product__buttons}>
                <Button title='Купить' styles={{mg: '0px 15px 0px 0px', bg_color: "#0a3241", color: "#fff"}} onclick={() => { buy(sizeUser) }} />
                {!isBasket ? <Button title='Добавить в корзину' styles={{
                    bg_color:'#4dd4f5',
                    color: "#222"
                }} onclick={() => { addBasket(sizeUser) }} /> : <Button title='Убрать из корзины' styles={{bg_color:'#ccc', color: "#222"}} onclick={deleteBasket} />}
            </div>
        </div>
    </div>
}
